function score = Neural_Network_batch_VolumeAndSGPTL(numHidden1, numHidden2, activation_function, lambda, beta, delta, R, rho, tau0, tau_p, tau_f, tau_min, m_ss,  patience, tolerance, init_w)
    ...
    % SGPTL Rule parameters
    % beta                                      % scalar value to compute deflected subgradient
    % delta                                     % value to define the optimal estimation
    % R                                         % distance covered until update delta with lower estimation
    %rho                                        % rate to reduce delta
    ...
    for fold = 1:k
        ...
        while epoch < maxEpochs
            ...
            [f_ref, f_best, delta, r] = SGPTLRule(f_ref, f_best, delta, r, R, rho, loss, alpha, d_curr);
    ...
    if avg_best_val < 0.7
        filename = sprintf( ...
                'models/SGPTL/stepsize/SGPTL-h1-%d-h2-%d-lambda-%g_%d.mat', ...
...
function [alpha, d_curr] = StepsizeRestricted(eps_d, sigma, alpha_prev, d_prev, g, gamma_prev, tau, beta, f_ref, delta, loss, epoch)
    ...
    % Stepsize()
    numeratore = max(0, beta_eff * (loss - (f_ref - delta)));
    denominatore = norm(d_curr)^2 + 1e-9;
    alpha = max(numeratore / denominatore, 0);
end

function [f_ref, f_best, delta, r] = SGPTLRule(f_ref, f_best, delta, r, R, rho, loss, alpha, d_curr)
