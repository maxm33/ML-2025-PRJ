function score = Neural_Network_batch_stepsizeRestrictedSGPTL_retraining(numHidden1, numHidden2, lambda, beta, delta_multiplicator, R, rho)
    %% ===================================
    ...
    data = load('models/SGPTL/stepsize/stepsizeRestrictedSGPTL-h1-70-h2-40-lambda-0.0001-beta-0.2-R-1-rho-0.5-deltaMult-0.1_299173.mat');
    model_sel = data.model;

    [~, best_run] = min(model_sel.mee_validation);
    % Early Stopping
    patience = 300;              
    tolerance = 0.0001;                           % minimum of 0.1% progress in patience epochs
    maxEpochs = 10000;
    ...
    for fold = 1:k
        ...
        W1 = model_sel.initial_weights.W1{best_run};
        W2 = model_sel.initial_weights.W2{best_run};
        W3 = model_sel.initial_weights.W3{best_run};
        ...
        num_params = numel(W1) + numel(W2) + numel(W3);
        ...
        while epoch < maxEpochs
            ...
            % W1
            nElem = numel(W1);
            W1 = W1 - alpha * reshape(d_vec(curr + (1:nElem)), size(W1));
            curr = curr + nElem;
            
            % b1
            nElem = numel(b1);
            b1 = b1 - alpha * reshape(d_vec(curr + (1:nElem)), size(b1));
            curr = curr + nElem;
            
            % W2
            nElem = numel(W2);
            W2 = W2 - alpha * reshape(d_vec(curr + (1:nElem)), size(W2));
            curr = curr + nElem;
            
            % b2
            nElem = numel(b2);
            b2 = b2 - alpha * reshape(d_vec(curr + (1:nElem)), size(b2));
            curr = curr + nElem;
            
            % W3
            nElem = numel(W3);
            W3 = W3 - alpha * reshape(d_vec(curr + (1:nElem)), size(W3));
            curr = curr + nElem;
            
            % b3
            nElem = numel(b3);
            b3 = b3 - alpha * reshape(d_vec(curr + (1:nElem)), size(b3));
    ...
    if avg_best_val < 0.65
        ...
        title(sprintf('Learning Curve | h1=%d; h2=%d; lambda=%g; valRMSE=%.3f', ...
            numHidden1, numHidden2, lambda, avg_best_val));
        grid on;
