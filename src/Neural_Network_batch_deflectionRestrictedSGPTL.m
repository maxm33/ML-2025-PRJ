function score = Neural_Network_batch_deflectionRestrictedSGPTL(numHidden1, numHidden2, lambda, beta, delta_multiplicator, R, rho)
    %% ===================================
    ...
    for epoch = 1:epochs
        ...
        %% Weights Update
        loss = total_error / P; %loss MEE che uso per calcolare alpha
        g = [
            grad_W_h1(:);
            grad_b_h1(:);
            grad_W_h2(:);
        ...
        if epoch == 1
            ...
            if f_best < loss  && alpha_prev > 0 %applico la restrizione solo se il milgioramento non è stato più di quello atteso
                %altrimenti gamma diventa negativo e con lui beta e alpha
        ...
        if(epoch == 1)
            f_ref =  loss;
            f_best =  loss;
            delta = delta_multiplicator* loss; %delta_multiplicator fattore di moltiplicazione per 
            %calcolare la percentuale di loss per delta
            %non soddisfaciente
            r = 0;
            %beta = 0.0015;
        ...
        % Hidden1 -> Hidden2
        for j = 1:numHidden2
            hidden_layer2(j).bias_weight = hidden_layer2(j).bias_weight + alpha * d_b_h2(j) / P;
            for i = 1:numHidden1
                hidden_layer2(j).input_connections(i).weight = ...
                    hidden_layer2(j).input_connections(i).weight + alpha *  d_W_h2(j,i) / P - ...
        ...
        % Hidden2 -> Output
        for k = 1:M
            output_layer(k).bias_weight = output_layer(k).bias_weight + alpha * d_b_out(k) / P;
            for j = 1:numHidden2
                output_layer(k).input_connections(j).weight = ...
                    output_layer(k).input_connections(j).weight + alpha * d_W_out(k,j) / P - ...
    ...
    filename = sprintf( ...
        'models/deflectionRestrictedSGPTL-h1-%d-h2-%d-lambda-%g-beta-%g-R-%g-rho-%g-deltaMult-%g_%d.mat', ...
    ...
    title(sprintf('Learning Curve | h1 = %d; h2 = %d; lambda = %g, beta = %g, R = %g, rho = %g, deltaMul = %g; Batch', ...
        numHidden1, numHidden2, lambda, beta, R, rho, delta_multiplicator));
    grid on;
